...
funkin::Script::Script(const std::string &path) {
    ...
    switch (result) {
        case WREN_RESULT_COMPILE_ERROR:
            TraceLog(LOG_ERROR, "Compile Error!");
            break;
        case WREN_RESULT_RUNTIME_ERROR:
            TraceLog(LOG_ERROR, "Runtime Error!");
            break;
...
funkin::Script::~Script() {
    TraceLog(LOG_INFO, "Shutting down Script...");
    wrenFreeVM(vm);
}
...
void funkin::Script::errorFn(WrenVM* vm, const WrenErrorType errorType, const char* module, const int line, const char* msg) {
    switch (errorType) {
        case WREN_ERROR_COMPILE:
            TraceLog(LOG_ERROR, TextFormat("[%s line %d] %s", module, line, msg));
            break;
        case WREN_ERROR_STACK_TRACE:
            TraceLog(LOG_ERROR, TextFormat("[%s line %d] in %s", module, line, msg));
            break;
        case WREN_ERROR_RUNTIME:
            TraceLog(LOG_ERROR, TextFormat("[Runtime Error] %s", msg));
            break;
    }
}
